...
def hough_line(img, angle_step=1):
  ...
  width, height, inutil = img.shape
  diag_len = np.int(np.sqrt(width * width + height * height))
  ...
  y_idxs, x_idxs, z_idxs = np.nonzero(img)  # (row, col) indexes to edges
